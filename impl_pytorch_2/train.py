...
def main(config, resume):
    ...
    classify_model = get_instance(module_arch, 'arch', config)
    ...
    trainable_params = filter(lambda p: p.requires_grad, classify_model.parameters())
    optimizer = get_instance(torch.optim, 'optimizer', config, trainable_params)
    lr_scheduler = get_instance(torch.optim.lr_scheduler, 'lr_scheduler', config, optimizer)

    trainer = Trainer(classify_model, loss, metrics, optimizer, 
                      resume=resume,
                      config=config,
